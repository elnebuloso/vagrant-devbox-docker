...
if [ ! -f "/vagrant/.ssh/root/id_rsa" ]; then
    ...
    puttygen /vagrant/.ssh/root/id_rsa -O private -o /vagrant/.ssh/root/id_rsa.ppk -C "vagrant-devbox-docker"
fi;
...
find /vagrant/.ssh/ -type f -name '*.pub' -exec cat {} \; -exec echo "" \; > /root/.ssh/authorized_keys
